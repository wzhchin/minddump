...
Exporting stored entries — text, comments, media, imported files — out of the MindDump vault to other Android apps via `ACTION_SEND` / `ACTION_SEND_MULTIPLE`, including transparent decryption of Private-space `.enc` entries, bulk sharing from multi-select, and whole-group sharing.
## Requirements
